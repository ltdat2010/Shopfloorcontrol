...
namespace Production.Class
{
    public partial class F_LOCATION : UC_Base
    {
        ...
        private void ItemClickEventHandler_Delete(object sender, EventArgs e)
        {
            ...
            {
                LOC.LOCCode = gridView1.GetFocusedRowCellValue("LOCCode").ToString();

                DialogResult dlDel = XtraMessageBox.Show(" Bạn muốn xóa khu vực mã : " + LOC.LOCCode + " ? ", "Xóa thông tin", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (dlDel == DialogResult.Yes)
                {
                    LOCBUS.LOCATION_DELETE(LOC);
